...
- **Wave:** 3
- **Task:** `FF-AI-DOC-005` (a crear en Wave 3)
...
1. **`docs/archive/source-material/`**: 
   - Confirmar que contiene **solo** material de diseño previo no canónico (actualmente: `roles-and-context-governance-design.md`, `roles-and-context-governance-source-material.md`).
...
3. **Reclasificación decidida (ADR-001)**: `docs/indexing-pipeline.md` → futuro destino `docs/research/semantic-retrieval.md` (movimiento lo ejecuta WP2; WP5 define policy de indexación/no-canonicidad).
...
- **Contexto:** ADR-001 declara que `archive/source-material` es no canónico (precedencia 7) y puede curarse por TASK explícita. La sección Reference/Non-Canonical en SoT **ya fue creada por Foundation** (WP1); WP5 no escribe SoT. **WP6 finaliza indexación**. Incluye reclasificación `indexing-pipeline.md` ya decidida (movimiento por WP2).
...
- `FF-AI-DOC-005` (a crear en `docs/tasks/FF-AI-DOC-005/{TASK.md,PLAN.md}` en Wave 3).
